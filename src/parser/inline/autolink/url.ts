...
import { combine, some, bracket } from '../../../combinator';
...
export const url: AutolinkParser.UrlParser = source => {
  ...
  source = flag
    ? source.slice(1)
    : source;
  const [, rest = undefined] = some(combine<AutolinkParser.UrlParser>([bracket('[', ipv6, ']'), parenthesis, some(escsource, closer)]))(source) || [];
