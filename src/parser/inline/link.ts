...
import { SubParsers, Data, combine, loop, bracket, transform } from '../../combinator';
...
export const link: LinkParser = source => {
  ...
  return transform(
    bracket(
      '[',
      loop(combine<Data<typeof inline>, SubParsers<LinkParser>>([inline]), /^]\n?|^\n/),
      ']'),
    (ns, rest) => {
      ...
      return transform(
        bracket(
          '(',
          loop(combine<Data<typeof inline>, SubParsers<LinkParser>>([parenthesis, escsource]), /^\)|^\s(?!nofollow)|^\n/),
          ')'),
        (ns, rest) => {
