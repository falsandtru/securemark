...
const syntax = /^(<([a-z]+)>)/i;
const inlinetags = Object.freeze('small|cite|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|mark|ruby|rt|rp|bdi|bdo|ins|del|wbr'.split('|'));
assert(inlinetags.every(t => /[a-z]+/.test(t)));
assert(inlinetags.every(t => ['script', 'style', 'link', 'a', 'img'].indexOf(t) === -1));
assert(inlinetags.every(t => ['s', 'em', 'strong', 'q', 'u'].indexOf(t) === -1));
