...
export const paragraph: ParagraphParser = verify(block(source => {
  ...
  const [cs = []] = subsequence<ParagraphParser>([
    some(reference),
    some(combine<[ParagraphParser.HashtagParser, InlineParser]>([hashtag, inline])),
  ])(block.replace(invisibleline, '').trim()) || [];
