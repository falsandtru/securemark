...
export function bind(target: DocumentFragment | HTMLElement | ShadowRoot, settings: Settings): {
  ...
  settings = ObjectAssign(ObjectCreate(settings), {
  ...
  function* parse(source: string): Generator<Progress, undefined, undefined> {
    ...
    const context: MarkdownParser.Context = ObjectAssign(settings, {
      url: url ? new URL(url, url) : undefined,
    });
    ...
    for (const el of footnote(target, footnotes, context)) {
      assert(rev === revision);
      el
    ...
    for (const el of figure(target, footnotes, context)) {
      assert(rev === revision);
      el
