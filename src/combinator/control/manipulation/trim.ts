...
export function trim<T, D extends Parser<unknown, any>[]>(parser: Parser<T, D>): Parser<T, D> {
  return convert(source => source.trim(), parser);
}

export function trimStart<P extends Parser<unknown, any>>(parser: P): P;
export function trimStart<T, D extends Parser<unknown, any>[]>(parser: Parser<T, D>): Parser<T, D> {
  return convert(
    source =>
...
export function trimEnd<T, D extends Parser<unknown, any>[]>(parser: Parser<T, D>): Parser<T, D> {
  return convert(
    (source, str?: string) =>
