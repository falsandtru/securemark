...
describe('Unit: combinator/subsequence', () => {
  describe('subsequence', () => {
    ...
    const c: Parser<string, never, object> = (source, config): Result<string, never, object> => {
      return source && source[0] === 'c'
        ? [['C'], source.slice(1), config]
        : undefined;
    }
    const abc = subsequence<Parser<string, [typeof a, typeof b, typeof c], object>>([a, b, c]);
