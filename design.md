...
上位区分の構文は下位区分の構文にパースの成否を左右されてはならない。
よって上位区分の構文は下位区分の構文の使用を検査してはならない。
また禁止する構文はその構文がないものとしてパースしなければならない。

ゆえにすべてのブロック構文は任意のインライン構文の使用を検査しない。
またすべての構文は任意の包含関係において無効化および有効化できる。

### 構文冗長性

構文の消費する文字列ならびに正常なパース結果は構文拡張の前後において不変でなければならない。
よって構文の種類や属性等のパラメータは値でなく書式で制約しなければならない。
また不正なパラメータ構成の入力文字列は誤入力および出力の将来的可変性を通知するパース結果を出力しなければならない。
