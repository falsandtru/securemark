...
さらにMarkdownテキスト全体のシンタックスハイライトは数十万以上の文字数では入力遅延がTextareaより大幅に増大する問題が予想される。この問題はエディタの基本構成単位をラインでなくブロックとし表示範囲内のブロックのみシンタックスハイライトを行うことで解決できるだろう。この場合ASTはブロック単位の独立したものでよいため前述の位置情報の考慮が不要となり容易に実装できる。エディタも既存の一般的な実装方法を簡素化するだけで足りる。このブロック単位の分割処理による入力ブロックのレイアウト計算の高速化によりTextareaで処理不能な数百万文字以上のMBサイズのテキストを処理できる可能性もある。
