...
~~さらにMarkdownテキスト全体のシンタックスハイライトは数十万以上の文字数では入力遅延がTextareaより大幅に増大する問題が予想される。この問題はエディタの基本構成単位をラインでなくブロックとすることで解決できるだろう。この場合ASTはブロック単位の独立したものでよいため前述の位置情報の考慮が不要となり容易に実装できる。エディタも既存の一般的な実装方法を簡素化するだけで足りる。このブロック単位の分割処理による入力ブロックのレイアウト計算の高速化によりTextareaで処理不能な数百万文字以上のMBサイズのテキストを処理できる可能性もある。~~

~~現在Textareaで50万文字でも入力遅延しなくなったため性能的にはTextareaで足りるようになった。あとはスクロール同期のためにTextareaの可視文字列範囲を知る必要があるがこのためのAPIがないため標準APIを追加する必要がある。~~

CodeMirrorの既成の簡素なMarkdownエディタを試すことができたがテキストでも満足のいく速さだったのでまずはASTを介さずテキストでこれを利用し追々互換性のあるASTまたは変換器を実装してよいかもしれない。VSCodeなどとも互換性を取りたかったのでASTの設計を遅らせてきたが遅いVSCodeに対応させるのはもうあきらめるのでCodeMirrorだけでいい。
...
またCommonMarkは拡張構文を標準化した次期標準仕様との互換性を確保させる役には立たずその有用性は構文の拡張を考慮しない範囲での効率的な実装方法の例示およびテストケースの集積による個別実装の支援ならびにその結果としての限定的互換性にとどまる。
