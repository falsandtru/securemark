...
構文数と使用記号数が大きく異なるため単純な比較はできないが直接比較してもにブラウザによるHTMLパース時間を含めたCommonMarkとの速度差は4倍、CommonMarkの問題の解決を試みたdjotとの速度差は等倍程度であり本質的な問題は存在しない。
Securemarkはパーサーコンビネータの使用によるオーバーヘッドが大きく、パーサーを展開しオーバーヘッドを削減すれば1.5-2倍の高速化が見込める。
